...
public abstract class Task<T> {
    ...
    public Task<T> withStagesHints(String... hints) {
        return withStagesHints(Arrays.stream(hints).map(StagesHint::new).collect(Collectors.toList()));
    }
    ...
    private static String getCaller() {
        ...
        for (StackTraceElement frame : stackTrace) {
            ...
            if (!className.startsWith(PACKAGE_PREFIX) && !"java.lang.Thread".equals(className)) {
                String fileName = frame.getFileName();
                if (fileName != null)
                    return className + '.' + frame.getMethodName() + '(' + fileName + ':' + frame.getLineNumber() + ')';
                else
