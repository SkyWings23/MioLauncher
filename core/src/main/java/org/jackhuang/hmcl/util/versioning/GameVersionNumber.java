...
import org.jackhuang.hmcl.util.io.MioAssets;
...
public abstract sealed class GameVersionNumber implements Comparable<GameVersionNumber> {
    ...
    static final class Versions {
        ...
        static {
            ...
            try (var reader = new BufferedReader(new InputStreamReader(MioAssets.open("/assets/game/versions.txt"), StandardCharsets.US_ASCII))) {
            ...
            //noinspection DataFlowIssue
            try (var reader = new BufferedReader(new InputStreamReader(MioAssets.open("/assets/game/version-alias.csv"), StandardCharsets.US_ASCII))) {
                for (String line; (line = reader.readLine()) != null; ) {
                    if (line.isEmpty())
                        continue;
