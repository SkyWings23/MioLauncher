...
public abstract class ObservableSetting implements Observable {
    ...
    private static final java.util.concurrent.ConcurrentHashMap<Class<?>, List<? extends ObservableField<?>>> FIELDS = new java.util.concurrent.ConcurrentHashMap<>();

    private static List<? extends ObservableField<?>> computeFields(@NotNull Class<?> type) {
    ...
    private static List<? extends ObservableField<?>> getFields(Class<?> type) {
        return FIELDS.computeIfAbsent(type, ObservableSetting::computeFields);
    }
    ...
    protected final void register() {
        ...
        @SuppressWarnings("unchecked")
        var fields = (List<ObservableField<ObservableSetting>>) getFields(this.getClass());
    ...
    public static abstract class Adapter<T extends ObservableSetting>
        ...
        public JsonElement serialize(T setting, Type typeOfSrc, JsonSerializationContext context) {
            ...
            @SuppressWarnings("unchecked")
            var fields = (List<ObservableField<T>>) getFields(setting.getClass());
