 ...
 */
package org.jackhuang.hmcl.addon.datapack;
...
public class DataPack {
        ...
        if (!containsMultiplePacks) {
            ...
            try (FileSystem outputResourcesZipFS = CompressingUtils.createWritableZipFileSystem(targetResourceZipPath);
                ...
                String metaContent = """
                        ...
                        """;
                try (var writer = Files.newBufferedWriter(packMcMeta, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)) {
                    writer.write(metaContent);
                }
                // Android desugar 兼容：确保目录存在
                if (packMcMeta.getParent() != null) {
    ...
    private Optional<Pack> parsePack(Path dataPackPath, boolean isDirectory, String name, Path mcmetaPath) {
        try {
            PackMcMeta mcMeta = JsonUtils.fromNonNullJson(AndroidFiles.readString(mcmetaPath), PackMcMeta.class);
