    ...
    public @NotNull String getVersionPageUrl(RemoteAddon.Version version) throws IOException {
        ...
        try {
            ...
            var clazz = switch (classId) {
                ...
                default ->
                        throw new IllegalArgumentException(String.format("Unsupported CurseForge class id [%d]", classId));
            };
            return String.format("%s/minecraft/%s/%s/files/%s", BASE, clazz, addon.slug(), version.versionId());
        } finally {
            SEMAPHORE.release();
        }
    }
