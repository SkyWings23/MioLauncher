...
public final class ForgeNewModMetadata {
    ...
    private static LocalModFile fromFile0(
        ...
        if (tomlParseResult.hasErrors()) {
            var ioException = new IOException("Mod " + modFile + String.format(" `%s` is malformed..", modToml.getName()));
        ...
        if (metadata == null || metadata.getMods().isEmpty())
            throw new IOException("Mod " + modFile + String.format(" `%s` is malformed..", modToml.getName()));
    ...
    private static ModLoaderType analyzeLoader(TomlParseResult toml, String modID, ModLoaderType loader) {
        ...
        try {
            ...
            if (tomlArray != null) {
                dependencies = tomlArray.toList().stream().map( o -> ((TomlTable) o).toMap()).collect(Collectors.toList());
            }
