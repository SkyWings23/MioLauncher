...
public class DefaultGameRepository implements GameRepository {
    ...
    public Collection<GameInstanceManifest> getInstanceManifests() {
        return status.instances.values().stream().map(i -> i.manifest).collect(Collectors.toList());
    }
    ...
    public Path getLibraryFile(GameInstanceManifest manifest, Library lib) {
        ...
        Path standard = getLibrariesDirectory(manifest).resolve(lib.getPath());
        if (Files.isRegularFile(standard)) {
            return standard;
        }

        // 库文件可能缓存在 cache/SHA-1（HMCL 下载缓存机制），回退到缓存。
        try {
            org.jackhuang.hmcl.util.CacheRepository repo = org.jackhuang.hmcl.util.CacheRepository.getInstance();
            if (repo instanceof org.jackhuang.hmcl.download.DefaultCacheRepository cacheRepo) {
                return cacheRepo.getLibrary(lib).orElse(standard);
            }
