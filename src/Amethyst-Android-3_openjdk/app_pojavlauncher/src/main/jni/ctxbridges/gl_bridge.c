...
gl_render_window_t* gl_init_context(gl_render_window_t *share) {
    ...
    LOGI("MioGL: eglCreateContext es=%d ctx=%p err=0x%x", libgl_es, (void*)bundle->context, eglGetError_p());
    if (bundle->context != EGL_NO_CONTEXT) {
        EGLint ver[2];
        if (eglQueryContext_p(g_EglDisplay, bundle->context, EGL_CONTEXT_CLIENT_VERSION, &ver[0])) {
            LOGI("MioGL: EGL context client version = %d", ver[0]);
        }
    }
...
void gl_swap_surface(gl_render_window_t* bundle) {
     ...
     */
    usleep(0); // MioLauncher: 去掉 750ms 等待，避免 initBackendSystem 因上下文未就绪而创建设备失败
...
void gl_make_current(gl_render_window_t* bundle) {
    ...
    LOGI("Making current, surface=%p, nativeSurface=%p, newNativeSurface=%p tid=%d", bundle->surface, bundle->nativeSurface, bundle->newNativeSurface, gettid());
    ...
    if(eglMakeCurrent_p(g_EglDisplay, bundle->surface, bundle->surface, bundle->context)) {
        ...
        LOGE("eglMakeCurrent returned with error: %04x (surface=%p, nativeSurface=%p) tid=%d", eglGetError_p(), bundle->surface, bundle->nativeSurface, gettid());
        if(hasSetMainWindow) {
