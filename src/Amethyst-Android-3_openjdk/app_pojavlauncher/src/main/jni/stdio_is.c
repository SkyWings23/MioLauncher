...
Java_net_kdt_pojavlaunch_utils_JREUtils_setupExitMethod(JNIEnv *env, jclass clazz,
    ...
    jclass cls = (*env)->FindClass(env,"net/kdt/pojavlaunch/ExitActivity");
    if ((*env)->ExceptionCheck(env)) (*env)->ExceptionClear(env);
    if (cls == NULL) {
        LOGW("setupExitMethod: ExitActivity not found, exit trap disabled");
        exitTrap_exitClass = NULL;
        exitTrap_staticMethod = NULL;
        return;
    }
    exitTrap_exitClass = (*env)->NewGlobalRef(env, cls);
