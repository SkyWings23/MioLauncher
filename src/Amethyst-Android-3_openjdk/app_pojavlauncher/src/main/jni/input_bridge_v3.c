...
static jmethodID getStaticMethodIfExists(JNIEnv *env, jclass clazz, const char *name, const char *sig);
...
jint JNI_OnLoad(JavaVM* vm, __attribute__((unused)) void* reserved) {
    if (pojav_environ->dalvikJavaVMPtr == NULL) {
        ...
        jclass cb = (*dvEnv)->FindClass(dvEnv,"org/lwjgl/glfw/CallbackBridge");
        if (cb == NULL) {
            // MioLauncher: CallbackBridge 可能不在 ART classpath，跳过。
            ...
            pojav_environ->method_onDirectInputEnable = NULL;
            pojav_environ->method_getAndroidDPI = NULL;
            pojav_environ->method_notifyLauncher = NULL;
        } else {
            pojav_environ->bridgeClazz = (*dvEnv)->NewGlobalRef(dvEnv, cb);
            // MioLauncher: 各方法可能不存在（FCL lwjgl 用新版 API），逐个容错
            pojav_environ->method_accessAndroidClipboard = getStaticMethodIfExists(dvEnv, pojav_environ->bridgeClazz, "accessAndroidClipboard", "(ILjava/lang/String;)Ljava/lang/String;");
            pojav_environ->method_onGrabStateChanged = getStaticMethodIfExists(dvEnv, pojav_environ->bridgeClazz, "onGrabStateChanged", "(Z)V");
            pojav_environ->method_onDirectInputEnable = getStaticMethodIfExists(dvEnv, pojav_environ->bridgeClazz, "onDirectInputEnable", "()V");
            pojav_environ->method_getAndroidDPI = getStaticMethodIfExists(dvEnv, pojav_environ->bridgeClazz, "getAndroidDPI", "()F");
            pojav_environ->method_notifyLauncher = getStaticMethodIfExists(dvEnv, pojav_environ->bridgeClazz, "notifyLauncher", "(I[I)Z");
        }
...
// MioLauncher: 直接用 JNI 传入的 clazz 参数，不依赖 FindClass
// （进程内 JVM 上下文里 FindClass 可能失败 → vmGlfwClass/methodID 全为 NULL → updateWindowSize 崩溃）
JNIEXPORT void JNICALL Java_org_lwjgl_glfw_GLFW_nativeInitializeGLFWNativeBridge(JNIEnv* env, jclass clazz) {
    JNIEnv *vmEnv;
    (*pojav_environ->runtimeJavaVMPtr)->GetEnv(pojav_environ->runtimeJavaVMPtr, (void**) &vmEnv, JNI_VERSION_1_4);
    if (vmEnv == NULL) vmEnv = env;
    if (clazz != NULL) {
        pojav_environ->vmGlfwClass = (*vmEnv)->NewGlobalRef(vmEnv, clazz);
    }
    if (pojav_environ->vmGlfwClass == NULL) {
        printf("MioBridge: vmGlfwClass is NULL, native bridge methods unavailable\n");
        return;
    }
    ...
    // MioLauncher: FCL lwjgl 的 GLFW 可能没有这两个字段，容错为 NULL（调用方已做 NULL 防护）
    if ((*vmEnv)->ExceptionCheck(vmEnv)) (*vmEnv)->ExceptionClear(vmEnv);
    jobject keyDownBufferJ = field_keyDownBuffer != NULL ? (*vmEnv)->GetStaticObjectField(vmEnv, pojav_environ->vmGlfwClass, field_keyDownBuffer) : NULL;
    if ((*vmEnv)->ExceptionCheck(vmEnv)) (*vmEnv)->ExceptionClear(vmEnv);
    pojav_environ->keyDownBuffer = keyDownBufferJ != NULL ? (*vmEnv)->GetDirectBufferAddress(vmEnv, keyDownBufferJ) : NULL;
    ...
    if ((*vmEnv)->ExceptionCheck(vmEnv)) (*vmEnv)->ExceptionClear(vmEnv);
    jobject mouseDownBufferJ = field_mouseDownBuffer != NULL ? (*vmEnv)->GetStaticObjectField(vmEnv, pojav_environ->vmGlfwClass, field_mouseDownBuffer) : NULL;
    if ((*vmEnv)->ExceptionCheck(vmEnv)) (*vmEnv)->ExceptionClear(vmEnv);
    pojav_environ->mouseDownBuffer = mouseDownBufferJ != NULL ? (*vmEnv)->GetDirectBufferAddress(vmEnv, mouseDownBufferJ) : NULL;
}
...
void updateWindowSize(void* window) {
    // MioLauncher: NULL 防护（nativeInitializeGLFWNativeBridge 失败时这些字段为 NULL，直接调用会崩溃）
    if (pojav_environ->glfwThreadVmEnv != NULL
            && pojav_environ->vmGlfwClass != NULL
...
void pojavPumpEvents(void* window) {
    // MioLauncher: 世界渲染时 EGL 上下文可能不在当前线程（"No context is current" 崩溃根因）。
    // 仅当上下文丢失时重新绑定（避免每帧 eglMakeCurrent 开销导致卡顿）。
    extern EGLContext (*eglGetCurrentContext_p)(void);
    extern void (*br_make_current)(basic_render_window_t*);
    if (eglGetCurrentContext_p != NULL && eglGetCurrentContext_p() == NULL
            && br_make_current != NULL && pojav_environ->mainWindowBundle != NULL) {
        static int logged = 0;
        if (logged < 3) { logged++; LOGI("MioGL: context lost on tid=%d, re-making current", gettid()); }
...
JNIEXPORT jstring JNICALL Java_org_lwjgl_glfw_CallbackBridge_nativeClipboard(JNIEnv* env, __attribute__((unused)) jclass clazz, jint action, jbyteArray copySrc) {
    ...
    jstring pasteDst = NULL;
    // MioLauncher: methodID 可能为 NULL，跳过剪贴板回调避免 JNI abort
    if (pojav_environ->bridgeClazz != NULL && pojav_environ->method_accessAndroidClipboard != NULL) {
        pasteDst = convertStringJVM(dalvikEnv, env, (jstring) (*dalvikEnv)->CallStaticObjectMethod(dalvikEnv, pojav_environ->bridgeClazz, pojav_environ->method_accessAndroidClipboard, action, copyDst));
...
JNIEXPORT void JNICALL Java_org_lwjgl_glfw_CallbackBridge_nativeSetGrabbing(__attribute__((unused)) JNIEnv* env, __attribute__((unused)) jclass clazz, jboolean grabbing) {
    LOGI("MioDIAG: SetGrabbing = %d", (int) grabbing);
    TRY_ATTACH_ENV(dvm_env, pojav_environ->dalvikJavaVMPtr, "nativeSetGrabbing failed!\n", return;);
    // MioLauncher: methodID 可能为 NULL（CallbackBridge 方法缺失/签名不符），
    // 直接调用会导致 ART JNI abort（jmethodID was NULL）。跳过回调仅记录状态。
    if (pojav_environ->bridgeClazz != NULL && pojav_environ->method_onGrabStateChanged != NULL) {
...
Java_org_lwjgl_glfw_CallbackBridge_nativeEnableGamepadDirectInput(__attribute__((unused)) JNIEnv *env, __attribute__((unused))  jclass clazz) {
    ...
    if (pojav_environ->bridgeClazz != NULL && pojav_environ->method_onDirectInputEnable != NULL) {
...
JNIEXPORT jfloat JNICALL Java_org_lwjgl_glfw_CallbackBridge_nativeGetAndroidDPI(JNIEnv* env, __attribute__((unused)) jclass clazz) {
    ...
    if (pojav_environ->bridgeClazz == NULL || pojav_environ->method_getAndroidDPI == NULL) return 0.0f;
...
JNIEXPORT jboolean JNICALL Java_org_lwjgl_glfw_CallbackBridge_nativeNotifyLauncher(JNIEnv* env, __attribute__((unused)) jclass clazz, jint type, jintArray action) {
    ...
    // MioLauncher: methodID 可能为 NULL，跳过通知避免 JNI abort
    if (pojav_environ->bridgeClazz == NULL || pojav_environ->method_notifyLauncher == NULL) return JNI_FALSE;
...
void critical_send_key(jint key, jint scancode, jint action, jint mods) {
    if (pojav_environ->GLFW_invoke_Key && pojav_environ->isInputReady) {
        // MioLauncher: keyDownBuffer 可能为 NULL（FCL lwjgl 的 GLFW 无此字段），必须防护
...
void critical_send_mouse_button(jint button, jint action, jint mods) {
    LOGI("MioDIAG: mouseBtn %d action=%d", button, action);
    if (pojav_environ->GLFW_invoke_MouseButton && pojav_environ->isInputReady) {
        // MioLauncher: mouseDownBuffer 可能为 NULL（FCL lwjgl 的 GLFW 无此字段），必须防护
...
// ============ MioLauncher: 自有输入桥（不依赖 FCL CallbackBridge 协议） ============
JNIEXPORT void JNICALL
Java_com_miolauncher_backend_NativeInput_setInputReady(JNIEnv* env, jclass clazz, jboolean ready) {
    pojav_environ->isInputReady = (int) ready;
    if (ready) {
        // 队列模式：事件写队列由游戏线程 pump 分发，避免 ART 主线程直接回调导致 ANR
        pojav_environ->isUseStackQueueCall = 1;
    }
    LOGI("MioGL: NativeInput setInputReady=%d isStackQueue=%d", (int)ready, pojav_environ->isUseStackQueueCall);
}

JNIEXPORT void JNICALL
Java_com_miolauncher_backend_NativeInput_sendCursorPos(JNIEnv* env, jclass clazz, jfloat x, jfloat y) {
    pojav_environ->cursorX = x;
    pojav_environ->cursorY = y;
    LOGI("MioGL: NativeInput cursorPos %.0f,%.0f ready=%d invokeCb=%d sq=%d", x, y,
         pojav_environ->isInputReady, pojav_environ->GLFW_invoke_CursorPos != NULL,
         pojav_environ->isUseStackQueueCall);
    critical_send_cursor_pos(x, y);
}

JNIEXPORT void JNICALL
Java_com_miolauncher_backend_NativeInput_sendMouseButton(JNIEnv* env, jclass clazz, jint button, jint action, jint mods) {
    LOGI("MioGL: NativeInput mouseBtn %d action=%d ready=%d invokeCb=%d", button, action,
         pojav_environ->isInputReady, pojav_environ->GLFW_invoke_MouseButton != NULL);
    critical_send_mouse_button(button, action, mods);
}

JNIEXPORT void JNICALL
Java_com_miolauncher_backend_NativeInput_sendKey(JNIEnv* env, jclass clazz, jint key, jint scancode, jint action, jint mods) {
    critical_send_key(key, scancode, action, mods);
}

JNIEXPORT void JNICALL
Java_com_miolauncher_backend_NativeInput_sendScroll(JNIEnv* env, jclass clazz, jfloat x, jfloat y) {
    critical_send_scroll((double) x, (double) y);
}

JNIEXPORT void JNICALL
Java_com_miolauncher_backend_NativeInput_sendWindowSize(JNIEnv* env, jclass clazz, jint width, jint height) {
    critical_send_screen_size(width, height);
}

// FCL 新版 CallbackBridge 协议：nativeSendData(ZILjava/lang/String;)V。
// 游戏侧不依赖它输入（Launcher 侧才用），但防止 UnsatisfiedLinkError，做个安全 no-op。
// MioLauncher: 字符串格式待解码，先占位。
JNIEXPORT void JNICALL
Java_org_lwjgl_glfw_CallbackBridge_nativeSendData(__attribute__((unused)) JNIEnv* env, __attribute__((unused)) jclass clazz,
                                                  __attribute__((unused)) jboolean isDown, __attribute__((unused)) jint type,
                                                  __attribute__((unused)) jstring data) {
    LOGW("MioGL: nativeSendData called (type=%d, isDown=%d) - no-op", type, (int) isDown);
}
...
static void registerFunctions(JNIEnv *env) {
    // MioLauncher: 已提供完整 CallbackBridge（ART 侧自定义控件输入路径），
    // 启用 Amethyst 完整 CriticalNative/noncritical 注册表。
    ...
    if (bridge_class == NULL) {
        // MioLauncher: CallbackBridge 不存在时清异常跳过注册
    ...
    // MioLauncher: RegisterNatives 可能因方法不匹配失败，清异常避免崩溃
    if ((*env)->ExceptionCheck(env)) (*env)->ExceptionClear(env);
}

// MioLauncher: 容错获取静态方法，失败返回 NULL 且不抛异常
static jmethodID getStaticMethodIfExists(JNIEnv *env, jclass clazz, const char *name, const char *sig) {
    jmethodID m = (*env)->GetStaticMethodID(env, clazz, name, sig);
    if (m == NULL) {
        if ((*env)->ExceptionCheck(env)) (*env)->ExceptionClear(env);
        LOGW("CallbackBridge method %s not found, skipping", name);
    }
    return m;
}
