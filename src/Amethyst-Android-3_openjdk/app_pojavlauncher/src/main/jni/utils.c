...
JNIEXPORT void JNICALL Java_net_kdt_pojavlaunch_utils_JREUtils_setDalvikJavaVM(JNIEnv *env, jclass clazz) {
    JavaVM *vm;
    if ((*env)->GetJavaVM(env, &vm) == JNI_OK) {
...
JNIEXPORT void JNICALL Java_net_kdt_pojavlaunch_utils_JREUtils_redirectStdout(JNIEnv *env, jclass clazz, jstring path) {
...
// MioLauncher: FCL 注入回调 stub（FCLInjector 需要这些符号存在）
__attribute__((visibility("default"))) void pojavSetInjectorCallback(void *callback) {
    (void) callback;
}
__attribute__((visibility("default"))) void pojavSetHitResultType(void *type) {
    (void) type;
}
