    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getActionMasked()){
            ...
            case MotionEvent.ACTION_POINTER_UP: // 6
                ...
                // 呼出输入法开关：点击该按钮后自动打开系统键盘（游戏内输入指令/聊天）
                if(getProperties().openKeyboardOnClick){
                    android.util.Log.i("MioKB", "button " + getProperties().name + " openKeyboardOnClick=true");
                    net.kdt.pojavlaunch.MainActivity.switchKeyboardState();
                }
                break;
