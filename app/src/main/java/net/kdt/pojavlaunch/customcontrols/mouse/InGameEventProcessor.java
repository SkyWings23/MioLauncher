    ...
    public boolean processTouchEvent(MotionEvent motionEvent) {
        switch (motionEvent.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (LauncherPreferences.PREF_ENABLE_MOUSE_CLICK) checkGestures();
                break;
            case MotionEvent.ACTION_MOVE:
                mTracker.trackEvent(motionEvent);
                float[] motionVector = mTracker.getMotionVector();
                float mouseSpeed = Math.max(0.1f, LauncherPreferences.PREF_MOUSESPEED);
                float deltaX = (float) (motionVector[0] * mSensitivity * mouseSpeed);
                float deltaY = (float) (motionVector[1] * mSensitivity * mouseSpeed);
